...
NODOA* sartu_urtez_ordenaturik(NODOA* ptr_hasiera,NODOA* ptr_berria){
	...
	ptr_aux = ptr_hasiera;
	if (ptr_hasiera == NULL ||
			ptr_berria->elementua.edizio_urtea < ptr_hasiera->elementua.edizio_urtea   ){
		//2005 eta 1980
		//LEHEN POSIZIOAN posisioan sartu behar dot
		ptr_berria->ptr_hurrengoa = ptr_hasiera;
		ptr_hasiera = ptr_berria;
	}else{
		//AURREKO ELEMENTUA AURKITZEN
		while( ptr_aux->ptr_hurrengoa != NULL  &&
				ptr_berria->elementua.edizio_urtea > ptr_aux->ptr_hurrengoa->elementua.edizio_urtea ){
			ptr_aux = ptr_aux->ptr_hurrengoa;
		}
		if (ptr_aux->ptr_hurrengoa == NULL){
			//AZKENEGO POSIZIOAREN ONDOREN SARTU
			printf("Elementua azkenaren ondoren doie.\n");
			//2009
			ptr_aux->ptr_hurrengoa = ptr_berria;
			...
			ptr_aux->ptr_hurrengoa = ptr_berria;
			//2007->2009
			ptr_berria->ptr_hurrengoa = ptr_aurkitua;
			//ptr_aux->ptr_hurrengoa = ptr_aux->ptr_hurrengoa;
		}
	}
	return ptr_hasiera;
}
...
void sortu_nodo_bat_josu(NODOA* ptr_elementua){
	if (ptr_elementua != NULL){
		...
		//PRINTF, FEGTS , SSCANG
		printf("Diksaren  edizio urtea:");
		fgets(str, MAX_STR, stdin);
		sscanf(str, "%d", &(ptr_elementua->elementua.edizio_urtea));
